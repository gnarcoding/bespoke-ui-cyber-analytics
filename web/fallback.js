var __ignore = (() => {
  ...
  function detectHero(summary) {
    if (summary.top_ips && summary.top_ips.length > 0) {
      ...
      if (ipRatio > 0.65) {
        ...
        const topPort = Object.entries(summary.target_ports).sort((a, b) => b[1] - a[1])[0];
        return {
          type: "dominant_actor",
          ip: topIP,
          ipCount: topCount,
    ...
    const tagEntries = Object.entries(summary.attack_tags).sort((a, b) => b[1] - a[1]);
  ...
  function AtAGlance({ summary }) {
    const methodData = Object.entries(summary.methods).sort((a, b) => b[1] - a[1]).map(([label, value]) => ({ label, value }));
    const portData = Object.entries(summary.target_ports).sort((a, b) => b[1] - a[1]).map(([label, value]) => ({ label: ":" + label, value }));
    return h(
      "div",
      { className: "at-a-glance" },
      h(
        "div",
        { className: "at-a-glance-stats" },
  ...
  function AttackTagStrip({ summary }) {
    const tags = Object.entries(summary.attack_tags).sort((a, b) => b[1] - a[1]).map(([label, value]) => ({ label: label.replace(/_/g, " "), value }));
    if (tags.length === 0) return null;
    return h(TagChips, { tags, title: "Attack Signatures Detected" });
  }
  function RequestTimeline({ summary }) {
    const data = Object.entries(summary.time_buckets).map(([label, value]) => ({ label, value }));
  ...
  function URLClusters({ summary }) {
    const data = Object.entries(summary.url_clusters).sort((a, b) => b[1] - a[1]).map(([label, value]) => ({ label, value }));
    return h(BarChart, { title: "Traffic by Category", data, color: "#8b5cf6", maxBars: 12 });
  }
  function SignatureHits({ summary }) {
    const rows = Object.entries(summary.signature_hits).sort((a, b) => b[1] - a[1]).map(([id, count]) => ({
      id: "#" + id,
  ...
  function HeroSection({ summary, hero }) {
    if (hero.type === "dominant_actor") {
      ...
      const urlDisplay = hero.targetUrl ? hero.targetUrl.length > 40 ? hero.targetUrl.slice(0, 40) + "..." : hero.targetUrl : null;
      const dominantTag = Object.entries(summary.attack_tags).sort((a, b) => b[1] - a[1])[0];
      return h(
        "div",
    ...
    if (hero.type === "dominant_tag") {
      ...
      return h(
        "div",
        { className: "hero-section hero-tag" },
  ...
  function DominantActor({ summary, heroActive }) {
    ...
    return h(
      "div",
      { className: "chart-container panel-alert" },
      h("h3", { className: "chart-title" }, "Dominant Actor Detected"),
      h(
        "div",
        { className: "dominant-actor" },
        h(
          "div",
          { className: "dominant-actor-grid" },
          h(StatCard, {
            label: "Source IP",
            value: topIP,
            color: "#ef4444"
          ...
          h(StatCard, {
            label: "Target URL",
            value: targetUrlStr.length > 30 ? targetUrlStr.slice(0, 30) + "..." : targetUrlStr,
            color: "#f59e0b"
          }),
          topPort ? h(StatCard, {
            label: "Target Port",
  ...
  function ApplianceLoginFlood({ summary }) {
    ...
    const sigComment = Object.entries(summary.signature_comments).find(([, c]) => /fortinet|sonicwall|palo alto|vpn/i.test(c));
    return h(
      "div",
      { className: "chart-container panel-warn" },
      ...
      h(
        ...
        h(StatCard, {
          ...
          value: fmt(count),
          subtitle: sigComment ? "Matched: " + sigComment[1] : pct(count, summary.total_entries) + " of traffic",
          color: "#ef4444"
        })
      )
    );
  ...
  function CredentialHarvesting({ summary }) {
    ...
    const credUrls = summary.top_urls.filter(([u]) => /\.(env|git|aws|ssh|npm|pg|htpasswd|svn)|credentials|secrets|serviceAccount|appsettings|id_rsa/i.test(u)).map(([label, value]) => ({ label, value }));
    return h(
      "div",
      ...
      credUrls.length > 0 ? h(BarChart, { title: "", data: credUrls, color: "#ef4444", maxBars: 8 }) : h("p", { className: "panel-note" }, fmt(count) + " attempts to access credential files (.env, .git/config, secrets.json, etc.)")
    );
  }
  ...
  function WordPressProbes({ summary }) {
    ...
    const wpUrls = summary.top_urls.filter(([u]) => /wp-|xmlrpc|wp-json/i.test(u)).map(([label, value]) => ({ label, value }));
    return h(
      "div",
      { className: "chart-container" },
      h(
      ...
      wpUrls.length > 0 ? h(BarChart, { title: "", data: wpUrls, color: "#f59e0b" }) : h("p", { className: "panel-note" }, fmt(count) + " requests targeting WordPress endpoints")
    );
  }
  function PathTraversal({ summary }) {
    if (!hasTag(summary, "path_traversal")) return null;
    const count = summary.attack_tags.path_traversal;
    const travUrls = summary.top_urls.filter(([u]) => /\.\.\//i.test(u) || /%2e%2e/i.test(u)).map(([label, value]) => ({ label, value }));
    return h(
      "div",
      { className: "chart-container" },
      h(
      ...
      travUrls.length > 0 ? h(BarChart, { title: "", data: travUrls, color: "#f97316", maxBars: 5 }) : h("p", { className: "panel-note" }, fmt(count) + " path traversal attempts (../ sequences)")
    );
  }
  ...
  function FallbackDashboard({ summary }) {
    ...
    const dateLabel = summary.date_range[0] === summary.date_range[1] ? summary.date_range[0] : summary.date_range[0] + " \u2013 " + summary.date_range[1];
    ...
    return h(
      "div",
      { className: "dashboard" + (hero ? " dashboard-hero-mode" : "") },
      h(
        "header",
      ...
      hero ? h("div", { className: "compressed-grid" }, remainingPanels) : h(Fragment, null, ...remainingPanels),
      h(
        "footer",
